  ...
  },
  {
    ...
    Cell: ({ value }) => {
      if (value && value.length > 0) {
        // Map through products array and return div for each product info
        const productInfo = value.map((product, index) => (
          <div key={index}>
            {product.quantity}@ {product.dimensions}
            {product.weight ? `\t- ${product.weight}KG` : ''}
          </div>
        ));
        return <div>{productInfo}</div>;
      }
      return null; // Handle the case where products is not available or empty
    }
  },  
  {
...
export const BasicTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data, error } = await supabase.from('offersloose').select('*');
        if (error) {
          throw error;
        }
        setData(data);
  ...
  const tableInstance = useTable(
    {
      ...
      data: data,
      initialState: {
        sortBy: [{ id: 'id', desc: true }] // Sort by ID column in ascending order
      }
    },
    useGlobalFilter,
    useSortBy,
